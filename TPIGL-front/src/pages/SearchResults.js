...
const SearchResults = () => {
    ...
    useEffect(() =>{
        ...
        setAnnonces(Object.values( response))
    },[])
    ...
    return ( 
       <div className="SearchResults">
        ...
        {Annonces.map(item => (
                 <div className="grid-item" key={item.id}><Annonce picture={item.image} title={item.titre}location={item.wilaya+", "+item.commune} type={item.letype} category={item.categorie} surface={item.surface} date={item.date} price={item.prix} /></div>
             ))}
        </div>
